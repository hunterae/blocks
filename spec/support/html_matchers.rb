def trim_html(html, xpath_removals=nil, css_removals=[])
...
RSpec::Matchers.define :closely_resemble_html do |expected|
  ...
  match do |actual|
    @actual = trim_html(actual, @xpath_removals.to_a,
                                @css_removals.to_a)
    @expected = trim_html(expected, (@other_xpath_removals || @xpath_removals).to_a,
                                    (@other_css_removals || @css_removals).to_a)
    @expected == @actual
  end
